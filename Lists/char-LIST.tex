...
\NEW{PC}{\cBond}{
  \s\MYname	{Peter Smith}
  \s\MYfile	{Bond.tex}
  \s\MYnumber	{011}
  \s\MYdesc	{A handsome young man in an impeccably tailored white suit.}
  \MYsex	{\male}
  \s\MYplayer	{}
...
\NEW{PC}{\cGirl}{
  \s\MYname	{Janet Caldwell}
  \s\MYfile	{Girl.tex}
  \s\MYnumber	{012}
  \s\MYdesc	{A gorgeous young woman in a scintillating cocktail dress.}
  \MYsex	{\female}
  \s\MYplayer	{}
...
\NEW{PC}{\cThief}{
  \s\MYname	{George Conner}
  \s\MYfile	{Thief.tex}
  \s\MYnumber	{010}
  \s\MYdesc	{A handsome older man in an ill fitting sports jacket.}
  \MYsex	{\male}
  \s\MYplayer	{}
...
\NEW{PC}{\cMaster}{
  \s\MYname	{Angela Williamson}
  \s\MYfile	{Master.tex}
  \s\MYnumber	{011}
  \s\MYdesc	{An older woman in a classic black dress.}
  \MYsex	{\male}
  \s\MYplayer	{}
...
\NEW{PC}{\cOne}{
  \s\MYname	{Sheriff Bernard Jones}
  \s\MYfile	{}
  \s\MYdesc{The previous Prince's Sheriff and strong right hand man.}
  \s\MYplayer	{Richard Dupell}
}

\NEW{PC}{\cTwo}{
  \s\MYname	{The Gorgon}
  \s\MYfile	{}
  \s\MYdesc{An ancient and reclusive cult leader from Mt. Tam}
  \s\MYplayer	{Vivian Halbany}
}

\NEW{PC}{\cThree}{
  \s\MYname	{Baron David Lee}
  \s\MYfile	{}
  \s\MYdesc{An elder who claims all waters inside and around the Bay Area}
  \s\MYplayer	{Scott Hutchinson}
}

\NEW{PC}{\cFour}{
  \s\MYname	{The Goblin Queen}
  \s\MYfile	{}
  \s\MYdesc{An eccentric information broker as old as the city.}
  \s\MYplayer	{Kelsey Risi}
}

\NEW{PC}{\cFive}{
  \s\MYname	{High Priest Joshua Armstrong}
  \s\MYfile	{}
  \s\MYdesc{A political shark and one of the Prince's favorites}
  \s\MYplayer	{Matthew Fisher}
}

\NEW{PC}{\cSix}{
  \s\MYname	{Rachel Lynd}
  \s\MYfile	{}
  \s\MYdesc{Prince Leland's accountant}
  \s\MYplayer	{Acata Felton}
}

\NEW{PC}{\cSeven}{
  \s\MYname	{Marie Kable}
  \s\MYfile	{}
  \s\MYdesc{Childe of the Gorgon; acts as her agent in SF}
  \s\MYplayer	{Amy Russo}
}

\NEW{PC}{\cEight}{
  \s\MYname	{Nicholas Sharp}
  \s\MYfile	{}
  \s\MYdesc{Prince Leland's named heir; young and technology focused}
  \s\MYplayer	{Aaron Sunshine}
}

\NEW{PC}{\cNine}{
  \s\MYname	{Terrance Cadmus}
  \s\MYfile	{}
  \s\MYdesc{Childe of Joshua Armstrong; not adjusting well to the night court.}
  \s\MYplayer	{Frisco Del Rosario}
}

\NEW{PC}{\cTen}{
  \s\MYname	{Claire Mondego}
  \s\MYfile	{}
  \s\MYdesc{Childe of Marie Kable; mere weeks old}
...
\NEW{NPC}{\cDealer}{
  \s\MYname	{The Dealer}
  \s\MYnumber	{}
  \s\MYdesc	{A dealer dressed in a black and silver uniform}
  \s\MYsex	{\female}
}

\NEW{NPC}{\cPartner}{
  \s\MYname	{Anna-Marie Robinson}
  \s\MYnumber	{}
  \s\MYdesc	{}
  \s\MYsex	{\female}
}
